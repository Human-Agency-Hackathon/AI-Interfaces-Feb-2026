...
| | | 4. **Use rich persona data from DEEP_BRAINSTORM roles.** `BridgeServer.ts:350` currently gives all agents the same generic mission string. The `DEEP_BRAINSTORM` roles have detailed `persona` fields (e.g. "The Wild Ideator: enthusiastically uninhibited, quantity over quality"). The `brainstorm-process.json` has even richer `system_prompt_addendum` per role. These need to make it into the actual agent system prompt. Map each spawned agent's role ID to the matching `RoleDefinition` and pass the persona text through to `SystemPromptBuilder`. |
| | | **CLIENT WORKSTREAM — Process visualization** |
| | | 5. **Handle `stage:advanced` messages.** `main.ts` has NO handler for `stage:advanced`. The server sends it (ProcessController broadcasts via delegate). Add a `ws.on('stage:advanced', ...)` handler that updates a new stage progress UI component. |
| | | 6. **Build a Stage Progress Bar / Banner.** New DOM overlay (like PromptBar/QuestLog pattern). Shows: current stage name, stage X of Y, maybe a simple progress indicator. Updates on `stage:advanced`. Should be visible in the sidebar or as a top banner so the viewer always knows what phase the brainstorm is in. |
| | | 7. **Stage transition announcement.** When `stage:advanced` fires, show a brief visual announcement — either a DialogueLog entry ("Stage complete: Divergent Thinking. Entering: Convergent Thinking...") or a brief overlay/toast. Something so the viewer notices the phase change. |
| | | 8. **Surface findings in DialogueLog.** `findings:posted` currently goes to `console.log` only (`main.ts:94`). Dispatch it to the DialogueLog panel so viewers can actually read the ideas being generated. This is the core content of the brainstorm — it needs to be visible. |
| | | 9. **Show agent role names.** When agents join during a brainstorm, their `role` field has the persona name (e.g. "The Wild Ideator"). Make sure this shows up on the agent sprite label and in dialogue log entries, not just the generic agent ID. |
| | | **PRIORITY ORDER:** Items 1-4 (server) are the gating work — without them the brainstorm only runs 3 stages with generic prompts. Items 5-9 (client) can be done in parallel by anyone. Items 5+8 are highest value on the client side since they make the brainstorm watchable. |
| | | **NOT needed yet (phase 2):** Human gate commands (/approve, /inject, /kill etc.), conditional branches, fast_demo_mode toggle, groupthink isolation, web search integration. Get the linear 7-stage flow working end-to-end first. |
