...
| | | | ✅ **RESOLVED:** S1-S7 + C1-C3 all done. See Behrang's entries below. |
...
| | | | ✅ **RESOLVED (@Behrang):** All 5 Behrang issues fixed. ✅ **RESOLVED (@Ida):** (1) DialogueLog listener leak — moot, file deleted by Ken (speech bubbles feature). (2) UIScene agentInfo Map leak — fixed, added `agent-left` handler that deletes stale entries. |
...
| 2026-02-21 | 17:30 | Ken | **Speech bubbles feature COMPLETE — all 5 phases done.** Replaced DialogueLog right panel with per-agent speech bubbles above sprites (SpeechBubbleManager). Sidebar is now collapsible. Phase 4: floating announcements for stage transitions/findings/system messages. Phase 5: deleted ThoughtBubble.ts + DialogueLog.ts (dead code). 22 SpeechBubbleManager tests, 52 client tests total, 469 server tests — all pass. Build clean. **@Ida:** bubble styling and sidebar chrome may want your polish pass. **@Behrang:** no protocol changes; pure client-side. |
| 2026-02-21 | 17:40 | Behrang | **S5 state persistence done and pushed (process-aware resume).** `handleResumeRealm` now restores a running brainstorm process instead of always spawning Oracle. Changes: (1) Extracted `createProcessDelegate()` + `wireProcessControllerEvents()` helpers from `handleStartProcess` for DRY reuse. (2) Added `{ resumed?: boolean }` option to `spawnProcessAgents` — injects `resumeNote` in system prompt. (3) Process-aware branch in `handleResumeRealm`: if `processState.status === 'running'`, looks up template, calls `ProcessController.fromJSON()`, respawns stage agents with resume context. Falls back to Oracle if template missing or no running process. (4) Navigation state restoration from `worldState.navigationState` (activates when S3 lands). (5) 8 new tests in `BridgeServer.resume.test.ts`. All 469 server tests pass. Design doc: `docs/plans/2026-02-21-state-persistence-s5-design.md`. Plan: `docs/plans/2026-02-21-state-persistence-s5-plan.md`. **@Ken:** S1+S2+S5 done. S3/S4 are Jeff's. S6 (graceful shutdown) and S7 (auto-load) are next. C1-C3 (client localStorage) also on my plate per your reassignment. |
| 2026-02-21 | 17:50 | Behrang | **S7 state persistence done and pushed (auto-load last realm on startup).** Server now auto-resumes the last active realm when it restarts. Changes: (1) Added `lastActiveRealmId` field + 3 methods (`get`/`set`/`clear`) to RealmRegistry + IRealmRegistry + RedisRealmRegistry. (2) Set `lastActiveRealmId` in `handleLinkRepo`, `handleResumeRealm`, `handleStartProcess`. Clear in `handleRemoveRealm` and `close()`. (3) New public `autoLoadLastRealm()` on BridgeServer — reads last realm ID, loads WorldState, initializes subsystems, process-aware agent spawn. Falls back to onboarding on any failure. (4) Called from `index.ts` (non-blocking). (5) 5 RealmRegistry tests + 3 auto-load tests. All 477 server tests pass. Design + plan docs in `docs/plans/`. **@Ken:** S1+S2+S5+S7 all done. Server persistence is functionally complete for the features I own. S3/S4 (Jeff's) and S6 (graceful shutdown, depends on S4) remain. Next: C1-C3 (client localStorage + auto-resume). |
| 2026-02-21 | 18:00 | Ida | **Task 30 (BootScene retheme) done, Task 32 (landing page audit) done, UIScene bug fix done.** (1) Rethemed all BootScene terrain tiles from bright greens/grays to dungeon stone palette (plum-gray, dark mortar, torchlit accents). (2) Landing page CSS already conforms to design spec — no changes needed. (3) Fixed UIScene agentInfo Map leak: added `agent-left` handler + `agentInfo` Map for display names. DialogueLog listener leak is moot (file was deleted). All pushed to main. **Still open from Ida's list:** Tasks 27 (design tokens), 28 (reskin chrome), 29 (reskin onboarding), 31 (agent sprites), 33 (room differentiation). Jeff's designs for task 35 (onboarding) and 56 (post-process review) need Ida's HTML/CSS work. |
| 2026-02-21 | 17:46 | Behrang | **State persistence + code review bugs: ALL DONE.** Session audit confirmed all work from previous sessions is complete and committed. **(1) State persistence S1-S7 + C1-C3:** Every task done — types expanded, ProcessController serialization, WorldState serialization, debounced saves, process-aware resume, graceful shutdown, auto-load on startup, localStorage identity, server:info gamePhase, auto-resume on refresh. 477 server + 52 client tests pass. Progress tracker updated at `docs/features/20260221.02_state_persistence/04_progress_state_persistence.md`. **(2) Ken's code review bugs for @Behrang (5 items):** All resolved — advancing flag race condition guard, no listener accumulation (new instances created per realm), WS message handler now has separate try/catch for JSON parse vs internal errors, agentTurnCounts pruned per stage in advanceStage, PlayerMoveMessage/PlayerNavigateEnterMessage/PlayerNavigateBackMessage added to shared/protocol.ts. **(3) ZoomControls unit tests:** 8 tests committed (`client/src/__tests__/ui/ZoomControls.test.ts`). **Nothing unfinished or broken.** Next agent: look at Jeff's export format (option b — server sends markdown), demo narrator mode from onboarding-design.md, and post-process review screen handler. |
| 2026-02-23 | 21:30 | Behrang | **Oracle Router feature COMPLETE — 10 tasks, all merged to main.** The Oracle is now a real LLM agent — first spawned in every session via `player:submit`. It analyzes user input and routes between three activity types: (1) repo only → CODE_REVIEW with fantasy-themed software heroes, (2) problem only → brainstorm with existing personas, (3) both → code brainstorm. **What was built:** `OracleManager` class (lifecycle, inter-stage feeding, tool event wiring), 4 new protocol types (`player:submit`, `oracle:decision`, `hero:summoned`, `hero:dismissed`), `createOracleMcpServer` with 8 tools (SelectHeroes, SummonReinforcement, DismissHero, PresentReport + 4 shared), Oracle + code-review hero prompt modes in SystemPromptBuilder, CODE_REVIEW process template with 10 software hero personas and 6 stages, model routing (haiku for exploration, sonnet for reasoning, opus for synthesis), BridgeServer wiring with `handlePlayerSubmit`, client DialogueLog handlers with Oracle notification styling. 522 server + 42 client tests pass. Design: `docs/plans/2026-02-23-oracle-router-design.md`. Plan: `docs/plans/2026-02-23-oracle-router-plan.md`. Diagrams updated: system-overview, agent-lifecycle, ARCHITECTURE.md. **@Ken:** Oracle spawns at map center (60,60) as before; hero forts are assigned via generateFogMap. No visual changes needed. **@Jeff:** CODE_REVIEW template has 6 stages — reconnaissance, deep analysis, cross-reference, oracle review, synthesis, presentation. Heroes embed Claude Code skill methodologies (systematic-debugging, pr-test-analyzer, type-design-analyzer, code-review) in their personas. If you want to tune the stage flow or hero personas, edit `shared/process.ts` CODE_REVIEW definition. |
