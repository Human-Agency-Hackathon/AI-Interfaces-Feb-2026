...
| 2026-02-21 | Ken | **@Behrang: Round 2 gap analysis. 7 runtime bugs that will break the brainstorm E2E flow. All server-side, all your domain.** |
| | | **CRITICAL (flow won't start correctly without these):** |
| | | 1. **`processContext` never reaches `buildSystemPrompt()`.** `AgentSessionManager.ts:buildPromptForSession()` builds a `PromptContext` but never includes `config.processContext`. This means `SystemPromptBuilder` always takes the codebase-exploration path. Every brainstorm agent gets the wrong system prompt (no persona, no problem, no stage goal). **Fix:** Add `processContext: config.processContext,` to the object passed to `buildSystemPrompt()` in `buildPromptForSession()`. One line. |
| | | 2. **`CompleteStage` and `SealChamber` are not mentioned in brainstorm prompts.** `SystemPromptBuilder.ts:buildProcessPrompt()` line 79 only lists PostFindings and UpdateKnowledge in the TOOLS AVAILABLE section. Both tools exist in `createBrainstormMcpServer()` but agents don't know they can call them. Stages 6 (Synthesis) and 7 (Presentation) use `explicit_signal` completion and will **hang forever**. **Fix:** Add CompleteStage and SealChamber to the TOOLS AVAILABLE section with instructions to call them when the stage goal is met. |
| | | **HIGH (flow runs but produces wrong results or visual glitches):** |
| | | 3. **`advanceStage({})` wipes collected artifacts.** `ProcessController.ts:241` calls `delegate.onStageAdvanced(currentStage.id, {})` with empty artifacts. `WorldState.advanceStage()` line 116 does `collectedArtifacts[completedStageId] = { ...artifacts }` which **overwrites** anything collected via `setArtifact()` during the stage. The artifact threading chain is broken; later stages get empty `priorArtifacts`. **Fix:** Either pass the already-collected artifacts from `worldState.getProcessState().collectedArtifacts[currentStage.id]`, or change `advanceStage()` to merge instead of overwrite. |
| | | 4. **Dismissed agents leave ghost sprites.** `BridgeServer.ts` `dismissStageAgents` delegate (line 308) only calls `sessionManager.dismissAgent()`. It never calls `worldState.removeAgent()` or broadcasts `agent:left`. Old agents accumulate as ghost sprites on the client across stage transitions. **Fix:** Add `this.worldState.removeAgent(roleId)` and `this.broadcast({ type: 'agent:left', agent_id: roleId })` to the dismiss loop. |
| | | **MEDIUM (process doesn't end cleanly):** |
| | | 5. **Process completion broadcast is malformed.** `ProcessController.ts:267` emits `stage:completed` with `{ processId, problem, message }` but the protocol expects `{ stageId, artifacts, isFinal }`. Client has no listener for `stage:completed` at all. **Fix:** Emit a proper `StageCompletedMessage` with `isFinal: true`, and add a client handler. |
| | | 6. **`completeProcess()` is never called.** `WorldState.completeProcess()` exists but nothing invokes it. Process state stays `status: 'running'` forever. **Fix:** Call it when the final stage completes. |
| | | 7. **Single + explicit_signal stages may hang.** If a single-turn agent goes idle without calling CompleteStage, there's no mechanism to re-prompt it. After fixing Gap 2 (tool awareness), add a follow-up prompt for single + explicit_signal stages when the agent idles without completing. |
| | | **Priority order:** Fix 1 → 3 → 4 → 2 → 7 → 5 → 6. Gaps 1-4 get the core stage loop working. Gaps 2+7 unblock the final two stages. Gaps 5+6 give a clean ending. |
...
| 2026-02-21 | Jeff's agent | **@Behrang: one-line fix needed in `SystemPromptBuilder.ts`.** Brainstorm agents are hitting the filesystem during ideation because `Read`/`Glob`/`Grep` are in `allowedTools` and the system prompt never tells them not to use those tools. Claude's default is to gather context by reading files before responding — so agents explore the repo even when their job is pure idea generation. This causes groupthink (all agents anchor to the same codebase) and makes stages slower. Fix: add this line to the TOOLS AVAILABLE section in `buildProcessPrompt()` in `SystemPromptBuilder.ts`: `"Do NOT use Read, Glob, or Grep — generate ideas purely from your knowledge and the context above."` One line, immediate effect. Full analysis and diagrams at `docs/agent-task-solving-flow.md`. |
| 2026-02-21 | Ken | **@Behrang: While your agent is brainstorming maps, please have it produce a brief for the new map and visual layout.** The brainstorm should output a concrete design doc: what the map looks like during a brainstorm session, how rooms are laid out for each stage, how agents move between rooms on stage transitions, what the visual hierarchy is (which room is central, how parallel rooms are arranged), and how the map evolves as stages advance. Drop it in `docs/` as a brief with Mermaid diagrams. This becomes the spec Ida's agent works from for visual design and the spec your agent works from for MapGenerator changes. |
