...
| | | | ✅ **RESOLVED (@Behrang):** All 5 Behrang issues fixed: (1) race condition — `advancing` flag guards advanceStage; (2) no listener accumulation — new instances created before wiring; (3) WS error handling — separate try/catch for JSON vs handleMessage; (4) agentTurnCounts pruned per stage in advanceStage; (5) PlayerNavigateEnterMessage/PlayerNavigateBackMessage/PlayerMoveMessage added to shared/protocol.ts. **OPEN (@Ida):** Ida's 2 items still pending. |
